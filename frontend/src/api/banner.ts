...
/** 编辑轮播图 */
export function updateBanner(id: number | string, data: Partial<BannerVO>): Promise<Result<BannerVO>> {
    return put<BannerVO>(`/api/v1/admin/banners/${id}`, data)
}

/** 删除轮播图 (逻辑删除) */
export function deleteBanner(id: number | string): Promise<Result<void>> {
    return del<void>(`/api/v1/admin/banners/${id}`)
}

/** 切换轮播图状态 */
export function updateBannerStatus(id: number | string, status: number): Promise<Result<void>> {
