          ...
          <tr v-for="row in seckillList" :key="row.id">
            ...
            <td>
              <div class="table-actions">
                <button v-if="row.status === 'PENDING'" class="table-action-btn"
                  @click="openEditDialog(row as SeckillGoodsVO)">编辑</button>
                <button v-if="row.status === 'PENDING' || row.status === 'ACTIVE'" class="table-action-btn muted"
                  @click="handleCancel(row as SeckillGoodsVO)">取消</button>
                <button v-if="row.status === 'ENDED' || row.status === 'CANCELLED'" class="table-action-btn muted"
                  disabled>—</button>
              </div>
            </td>
          </tr>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="650px" :close-on-click-modal="false"
      ...
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px">
        <el-form-item label="商品" prop="productId">
          <el-select v-model="formData.productId" placeholder="请输入商品名称搜索" filterable remote
            :remote-method="searchProducts" :loading="productLoading" style="width: 100%" @change="handleProductChange">
        ...
        <el-form-item label="秒杀价格" prop="seckillPrice">
          <el-input-number v-model="formData.seckillPrice" :min="0.01" :precision="2" :step="1"
        ...
        <el-form-item label="秒杀库存" prop="stockCount">
          <el-input-number v-model="formData.stockCount" :min="1" :step="1" controls-position="right"
            style="width: 200px" />
        ...
        <el-form-item label="活动描述" prop="description">
          <el-input v-model="formData.description" type="textarea" :rows="3" placeholder="请输入活动描述" maxlength="200"
            show-word-limit />
        </el-form-item>
      </el-form>
      <template #footer>
