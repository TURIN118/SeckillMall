-- ============================================================
-- 高并发秒杀电商平台 - 初始化种子数据（脱敏示例）
-- 安全修复 B5/L-S6/M-F9：
--   1. 不含任何真实 PII，所有字段均为虚构示例数据
--   2. 不在注释中记录明文口令；口令通过 BCrypt(strength=10) 加密存储
--   3. admin/buyer01 口令哈希已验证与登录测试一致
--   4. 生产环境首次登录后应强制改密（口令轮换由部署流程负责）
-- ============================================================

-- ------------------------------------------------------------
-- 1. 用户数据（管理员/测试买家，口令已 BCrypt 加密，明文不入库不入注释）
-- ------------------------------------------------------------
INSERT INTO `t_user`
...
VALUES
  (1, 'admin', '$2a$10$EjQihKeAPFfMKPEuIp59OOK/2t2pss7114YVnk.xnZyXLi2bDvgKu',
   '13800000000', 'admin@example.com', '系统管理员', 'ADMIN', 'ACTIVE', 0),
  (2, 'buyer01', '$2a$10$W7HMDBLUe2KqbQ7lHuHVQO/ISol4Y/VZyLjF2PDcOf7RrflnqBFki',
   '13900000001', 'buyer01@example.com', '测试买家', 'BUYER', 'ACTIVE', 0);
